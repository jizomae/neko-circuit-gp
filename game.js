...
function beginRace(){initAudio();if(audioCtx.state==='suspended')audioCtx.resume();resetRace();menu.classList.remove('active');hud.classList.remove('hidden');document.querySelector('#touch-controls').classList.remove('hidden');state='countdown';countdown=3.8}
...
function updateCamera(dt){let r=racers[0],forward=new THREE.Vector3(Math.sin(r.rotation.y),0,Math.cos(r.rotation.y)),target=r.position.clone().add(new THREE.Vector3(0,2.8,0));let desired=target.clone().addScaledVector(forward,-10.5).add(new THREE.Vector3(0,5.5,0));camera.position.lerp(desired,1-Math.pow(.001,dt));let look=target.clone().addScaledVector(forward,8);camera.lookAt(look)}
